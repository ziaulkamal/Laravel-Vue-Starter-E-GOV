<template>
    <SimporaLayout :title="isEdit ? 'Edit Penginapan' : 'Tambah Penginapan'">
        <div class="page-wrap">
            <AppBreadcrumb :items="[
                { label: 'Penginapan', href: '/lodgings' },
                { label: isEdit ? 'Edit Penginapan' : 'Tambah Penginapan' }
            ]" />

            <div class="page-header">
                <h1 class="page-title">{{ isEdit ? 'Edit Penginapan' : 'Tambah Penginapan' }}</h1>
            </div>
            ...
            <form @submit.prevent="submit">
                <div class="form-sections">
                    ...
                    <AppCard>
                        <template #header><span class="section-title">Informasi Penginapan</span></template>
                        <div class="form-grid">
                            <div class="form-field--full">
                                <AppInput
                                    v-model="form.name"
                                    label="Nama Penginapan"
                                    placeholder="Contoh: Hotel Meukek"
                                    required
                                    :error="errors.name"
                                />
                            </div>
                    ...
                    <AppCard>
                        <template #header><span class="section-title">Penanggung Jawab (PIC)</span></template>
                        <div class="form-grid">
                            <AppInput
                ...
                <div class="form-actions">
                    ...
                    <AppButton variant="primary" type="submit" :loading="loading">Simpan Penginapan</AppButton>
                </div>
            </form>
        </div>
    </SimporaLayout>
...
// ── Wilayah cascade (mirip Venues/Form) ───────────────────────
...
async function fetchExisting() {
    ...
    try {
        const res = await api.get(`/api/v1/lodgings/${realId.value}`);
        const l = res.data?.data;
        if (!l) { notFound(); return; }
        form.name      = l.name ?? '';
        form.address   = l.address ?? '';
        form.capacity  = l.capacity ?? '';
        form.pic_name  = l.pic_name ?? '';
        form.pic_phone = l.pic_phone ?? '';
        form.latitude  = l.latitude ?? '';
        form.longitude = l.longitude ?? '';
        form.is_active = !!l.is_active;
        await populateWilayah(l.wilayah_kode ?? '');
...
function goBack() {
    if (isEdit.value && realId.value != null && !Number.isNaN(realId.value))
        router.visit(`/lodgings/${encodeId(realId.value)}`);
    else router.visit('/lodgings');
}
...
async function submit() {
    ...
    const payload = {
        ...
        pic_name:     String(form.pic_name).trim() || null,
        pic_phone:    String(form.pic_phone).trim() || null,
    ...
    loading.value = true;
    try {
        if (isEdit.value) await api.put(`/api/v1/lodgings/${realId.value}`, payload);
        else              await api.post('/api/v1/lodgings', payload);
        toast.success(isEdit.value ? 'Penginapan berhasil diperbarui' : 'Penginapan berhasil dibuat');
        router.visit('/lodgings');
    } catch (e: any) {
        ...
        if (status === 422 && body?.errors) {
            ...
            toast.error(body?.message ?? 'Gagal menyimpan penginapan');
        }
        loading.value = false;
    }
}
